...
def parse_args():
  ...
  parser.add_argument('--predictor_name', type=str, default='celeba',
...
def main():
  ...
  if predictions:
    if args.predictor_name == 'celeba':
      for key, val in predictions.items():
          predictions[key] = np.concatenate(val, axis=0)
    np.save(os.path.join(args.output_dir, 'scores_dict.npy'), predictions)
